...
import { getEnrolledCourseIds, getEnrolledCourses, getEnrolledCoursesById, getMentorCourses } from './courses';
...
export const assignmentsRouter = createTRPCRouter({
  ...
  getAssignmentDetailsByUserId: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    try {
      const currentUser = ctx.session.user;
  ...
  getAllAssignmentDetailsBy: protectedProcedure
    ...
    .query(async ({ ctx, input }): Promise<AssignmentDetails | { error: string }> => {
      try {
        const currentUser = ctx.session.user;
        ...
        const notSubmittedMentees = mentees.filter((mentee) => {
          return !assignment?.submissions.some((submission) => submission.enrolledUser.username === mentee.username);
        });
        ...
        const isCourseAdmin = currentUser.adminForCourses.some((course) => course.id === assignment?.courseId);
    ...
    .query(async ({ ctx, input }): Promise<AssignmentDetails | { error: string }> => {
      try {
        ...
        const notSubmittedMentees = allStudents.filter((student) => {
          return !assignment?.submissions.some((submission) => submission.enrolledUser.username === student.username);
        });
  ...
  getAllAssignmentsByCourseId: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    try {
      const currentUser = ctx.session.user;
  ...
  getMentorPieChartData: protectedProcedure.input(z.object({ courseId: z.string() })).query(async ({ ctx, input }) => {
    try {
      const currentUser = ctx.session.user;
      ...
      const notSubmitted = noOfTotalAssignments * noOfTotalMentees - assignmentsWithPoints - assignmentsWithoutPoints;
      ...
      try {
        ...
        return {
          ...
          totalPoints: assignments.reduce(
            (total, assignment) => total + assignment.points.reduce((sum, point) => sum + point.score, 0),
            0,
          ),
        };
      } catch (error) {
  ...
  getAssignmentDetails: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
    try {
      return await ctx.db.attachment.findUnique({
        where: {
          id: input.id,
  ...
  submitAssignment: protectedProcedure.input(z.object({ id: z.string() })).mutation(async ({ ctx, input }) => {
    try {
      const currentUser = ctx.session.user;
  ...
  getAssignmentDetailData: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        const currentUser = ctx.session.user;
        const { assignmentId, username, page, limit, selectedMentor, searchQuery } = input;
        ...
        const sortedAssignments = [...(assignmentData.submissions ?? [])].sort(
          (a: { enrolledUser: { username: string } }, b: { enrolledUser: { username: string } }) =>
        ...
        const isCourseAdmin =
          currentUser.role === 'INSTRUCTOR'
            ...
            : currentUser.adminForCourses.some((course: { id: string }) => course.id === assignmentData.course?.id);
          ...
          ? Array.from(
              new Set(
                assignmentData.course.enrolledUsers
                  .map((user: { mentorUsername: string | null }) => user.mentorUsername)
                  .filter(Boolean),
              ),
            )
          : [];
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        if (currentUser.role === 'STUDENT') {
          return { success: false, error: 'Students cannot access evaluation page' };
        }
        ...
        const submission = filteredSubmissions.find((submission) => submission?.id === submissionId);
