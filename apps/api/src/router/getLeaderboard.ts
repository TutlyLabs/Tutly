...
async function getLeaderboardDataForUser(userId: string, organizationId: string) {
  ...
  try {
    ...
    const totalPoints: Array<LeaderboardSubmission> = submissionsUptoLastSunday.map((submission) => {
...
export const leaderboardRouter = createTRPCRouter({
  ...
  getLeaderboardDataForStudent: protectedProcedure.query(async ({ ctx }) => {
    ...
    const result = await getLeaderboardDataForUser(currentUser.id, currentUser.organization.id);
    if (!result.success) {
      return result;
    }
