...
export default function CourseDetailsClient({
  ...
  const { data: assignmentsResponse, isLoading: assignmentsLoading } =
    api.attachments.getCourseAssignments.useQuery({
      courseId,
    });
                ...
                >
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <Link
                      ...
                      className="max-w-full cursor-pointer truncate text-base font-semibold text-blue-600 hover:text-blue-500"
                      title={attachment.title}
                    >
                      {attachment.title}
                    </Link>
                    <div className="gadiv-2 flex flex-shrink-0 items-center text-sm font-medium">
                      {attachment.dueDate &&
