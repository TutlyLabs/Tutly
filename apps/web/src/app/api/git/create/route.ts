...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!attachment || !attachment.course) {
            return NextResponse.json({ error: "Assignment or Course not found" }, { status: 404 });
        }

        const courseSlug = attachment.course.slug || `course-${attachment.course.id.slice(0, 8)}`;
        ...
        if (type === "TEMPLATE") {
            ...
            if (!enrollment || (enrollment.user.role !== "INSTRUCTOR" && enrollment.user.role !== "ADMIN")) {
                return NextResponse.json({ error: "Only enrolled instructors can create template repositories" }, { status: 403 });
            }
        ...
        if (type === "SUBMISSION") {
            if (!attachment.gitTemplateRepo) {
                return NextResponse.json({ error: "Template repo not ready" }, { status: 400 });
            }
            ...
            if (!enrolledUser || (enrolledUser.user.role !== "STUDENT" && enrolledUser.user.role !== "MENTOR")) {
                return NextResponse.json({ error: "Only enrolled students can create submission repositories" }, { status: 403 });
            }
            ...
            } else if (submission.status === "CREATED" && submission.gitRepoPath) {
                // Retrieve the session token to embed in the clone URL.
                const token = session.session?.token;
                const expiresAt = session.session?.expiresAt;
            ...
            await giteaClient.ensureUserExists(studentUsername, session.user.email || `${studentUsername}@tutly.in`);
            ...
            const [templateOwner, templateRepoName] = attachment.gitTemplateRepo.split("/");
            ...
            await giteaClient.ensureUserExists(owner, session.user.email || `${owner}@tutly.in`);
