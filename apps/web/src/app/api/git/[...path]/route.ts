...
async function handleGitRequest(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  const queryString = upstreamSearchParams.toString() ? `?${upstreamSearchParams.toString()}` : "";
  ...
  if (courseId) {
      ...
      if (type === "assignment" && req.method === "POST") {
          if (enrollment.user.role !== "INSTRUCTOR" && enrollment.user.role !== "ADMIN") {
              return new NextResponse("Forbidden: Only instructors can modify template repositories", { status: 403 });
          }
      }
      ...
      if (type === "submission" && req.method === "POST") {
          if (enrollment.user.role !== "STUDENT" && enrollment.user.role !== "MENTOR") {
              return new NextResponse("Forbidden: Only students can modify submission repositories", { status: 403 });
          }
      }
  }
  ...
  try {
      ...
      const allowedHeaders = ["content-type", "cache-control", "pragma", "expires", "date", "last-modified"];
