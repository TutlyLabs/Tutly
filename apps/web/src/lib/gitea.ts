...
export const giteaClient = {
  async createRepo(owner: string, name: string, isPrivate = true, isTemplate = false) {
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to create repo ${owner}/${name}: ${response.status} ${error}`);
  ...
  async generateRepoFromTemplate(
    ...
    if (!response.ok) {
      const error = await response.text();
      if (response.status === 409) return; // Repository already exists.
      console.error(`Failed to generate repo from template ${templateOwner}/${templateRepo} -> ${owner}/${name}: ${response.status} ${error}`);
  ...
  async checkRepoExists(owner: string, name: string) {
    const response = await fetch(`${GITEA_API_URL}/api/v1/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}`, {
      method: "GET",
      headers: {
        Authorization: `token ${GITEA_ADMIN_TOKEN}`,
      },
  ...
  async checkOrgExists(org: string) {
      const response = await fetch(`${GITEA_API_URL}/api/v1/orgs/${encodeURIComponent(org)}`, {
  ...
  async addUserToOrg(org: string, username: string) {
      ...
      const teamsResponse = await fetch(`${GITEA_API_URL}/api/v1/orgs/${encodeURIComponent(org)}/teams`, {
          headers: {
              Authorization: `token ${GITEA_ADMIN_TOKEN}`,
          }
      });
      ...
      if (teamsResponse.ok) {
          ...
          const defaultTeam = teams.find((t: any) => t.name === "members" || t.name === "Owners") || teams[0];
      ...
      if (!teamId) {
          const createTeamResponse = await fetch(`${GITEA_API_URL}/api/v1/orgs/${encodeURIComponent(org)}/teams`, {
              method: "POST",
              headers: {
                  Authorization: `token ${GITEA_ADMIN_TOKEN}`,
                  "Content-Type": "application/json",
          ...
          if (createTeamResponse.ok) {
              ...
              console.error(`Failed to create team for org ${org}: ${createTeamResponse.status} ${errorText}`);
              throw new Error(`Failed to create team`);
          }
      }
      
      // Add the user to the identified team.
      const addMemberResponse = await fetch(`${GITEA_API_URL}/api/v1/teams/${teamId}/members/${encodeURIComponent(username)}`, {
          method: "PUT",
          headers: {
              Authorization: `token ${GITEA_ADMIN_TOKEN}`,
          }
      ...
      if (!addMemberResponse.ok) {
          ...
          console.error(`Failed to add user ${username} to team ${teamId} in org ${org}: ${addMemberResponse.status} ${errorText}`);
  ...
  async ensureUserExists(username: string, email: string) {
      ...
      const check = await fetch(`${GITEA_API_URL}/api/v1/users/${encodeURIComponent(username)}`, {
          headers: { Authorization: `token ${GITEA_ADMIN_TOKEN}` }
      });
      ...
      if (!create.ok) {
          ...
          console.error(`Failed to create gitea user ${username}: ${create.status} ${err}`);
